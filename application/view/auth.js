...
Page.prototype.renderBlockContentInternal = function () { // @7:1
  ...
  self.writer.write(
    ...
    this.routePath('datatypes'), // @73:20
    '#ErrorCode_WRONG_TOKEN\'>WRONG_TOKEN</a>',
    '\n  ', // @73:91
    '}\n}</pre><p>В соединении с помощью веб-сокетов приходит событие <code><a href="',
    this.routePath('server2client'), // @77:71
    '#error">error</a></code> со следующим содержимым поля <code>error</code>:</p><pre js>{\n  code: <a href=\'',
    this.routePath('datatypes'), // @80:18
    '#ErrorCode_WRONG_TOKEN\'>WRONG_TOKEN</a>',
    '\n', // @80:89
    '}</pre><p>В случае ошибки нужно снова произвести процедуру авторизации.</p><h2>Выход</h2><div class=\'request\'><div class=\'method\'>DELETE</div><div class=\'path\'>/signout</div></div><p>Ответ:</p><pre js>{}</pre><h2>Регистрация</h2><div class=\'request\'><div class=\'method\'>POST</div><div class=\'path\'>/signup</div></div><p>Ответ:</p><pre js>{}</pre>'
  );
}; // @106:1
